-- KZN Route: Taxi Wars  v6
-- Map background · Region colour-coding · 5 KZN Regions
...
-------------------------------------------------------------------------------
-- REGIONS - 5 KZN regions with colours matching the map image
-------------------------------------------------------------------------------
local REGIONS = {
    {
        key    = "northern_natal",
        name   = "Northern Natal",
        colour = {0.95, 0.60, 0.10},   -- orange
        nodes  = {1, 2, 3, 19, 18, 12, 13},
        lx = 630, ly = 120,
    },
    {
        key    = "midlands",
        name   = "Midlands",
        colour = {0.25, 0.72, 0.28},   -- green
        nodes  = {4, 5, 6, 7, 8, 27, 28, 16},
        lx = 185, ly = 300,
    },
    {
        key    = "north_coast",
        name   = "North Coast",
        colour = {0.80, 0.10, 0.15},   -- red
        nodes  = {14, 15, 17, 11},
        lx = 555, ly = 350,
    },
    {
    ...
    },
    {
        key    = "south_coast",
        name   = "South Coast",
        colour = {0.15, 0.42, 0.85},   -- blue
        nodes  = {30, 26, 23, 22, 24, 20, 21},
        lx = 370, ly = 665,
    },
}

-- Build lookup: node id -> region table
local NODE_REGION = {}
for _, reg in ipairs(REGIONS) do
    for _, nid in ipairs(reg.nodes) do
        NODE_REGION[nid] = reg
    end
end
...
local SIDEBAR_W    = 260
local MAP_W        = WIN_W - SIDEBAR_W   -- 1020
local MAP_H        = WIN_H               -- 720

local fonts    = {}
local mapImage = nil   -- loaded in love.load if kzn_map.png is present

-------------------------------------------------------------------------------
-- NODES  (positions remapped to match actual KZN geography)
-------------------------------------------------------------------------------
local nodes = {
    -- NORTHERN NATAL (top-right orange area)
    ...
    -- MIDLANDS (left large green area)
    ...
    -- NORTH COAST (centre-right red area)
    ...
    -- DURBAN (right middle purple area)
...
-------------------------------------------------------------------------------
-- MAP TRANSFORM - auto-fits node coords to screen
-------------------------------------------------------------------------------
...
local function compute_transform()
    ...
    local rx = max_x - min_x
    local ry = max_y - min_y
    local px = rx + rx * 0.18
    local py = ry + ry * 0.18
    local cx = (min_x + max_x) / 2
    local cy = (min_y + max_y) / 2
    local aw = MAP_W - MAP_PAD * 2
    local ah = MAP_H - MAP_PAD * 2
    MS  = math.min(aw / px, ah / py)
    MOX = MAP_W / 2 - cx * MS
    MOY = MAP_H / 2 - cy * MS
end
...
local function update_valid_moves()
    ...
    for _, c in ipairs(get_connected(pos)) do
        if c.to ~= opp then table.insert(state.valid_moves, c.to) end
    end
end
...
local function apply_nkabi(player)
    if player==1 then state.p1_score=math.max(0,state.p1_score-2)
    else              state.p2_score=math.max(0,state.p2_score-2) end
...
local function apply_super_nkabi(player)
    ...
    if steps==0 then
        local sp = player==1 and 1 or 24
        if player==1 then state.p1_pos=sp else state.p2_pos=sp end
        flash_node(sp, 1.5)
        set_msg("💀💀 SUPER NKABI! P"..player.." sent back to start!", "bad", 2.5)
    else
        local tgt = trail[steps]
        if player==1 then state.p1_pos=tgt else state.p2_pos=tgt end
        flash_node(tgt, 1.5)
        set_msg("💀💀 SUPER NKABI! P"..player.." sent back "..steps.." nodes", "bad", 2.5)
    end
    if player==1 then state.p1_score=math.max(0,state.p1_score-1)
    else              state.p2_score=math.max(0,state.p2_score-1) end
end
...
local function check_win()
    local WIN_SCORE = 20
    if state.p1_score >= WIN_SCORE then state.game_over=true; state.winner=1
    elseif state.p2_score >= WIN_SCORE then state.game_over=true; state.winner=2 end
end
...
local function do_move(tid)
    ...
    if state.turn_blocked[state.current_player] then
        ...
        update_valid_moves(); return
    end

    if #state.valid_moves==0 then
        set_msg("STALEMATE! P"..state.current_player.." has no moves.", "warn", 2)
        state.current_player = state.current_player==1 and 2 or 1
        update_valid_moves(); return
    end
    ...
    if tid<1 or tid>30 then
        set_msg("Invalid — choose node 1-30","bad",2); return
    end

    local connected, route = is_connected(pos, tid)
    if not connected then
        set_msg("Node "..tid.." not connected!","bad",2); return
    end
    if tid==opp then
        set_msg("Node occupied by opponent!","bad",2); return
    end

    push_trail(cp, pos)
    if cp==1 then state.p1_pos=tid else state.p2_pos=tid end
    flash_node(tid, 0.9)
    route.cleared = true
    ...
    if node.customers > 0 then
        ...
        node.customers = 0
        if cp==1 then state.p1_score=state.p1_score+got
        else           state.p2_score=state.p2_score+got end
        local reg   = NODE_REGION[tid]
        local rname = reg and (" ["..reg.name.."]") or ""
        set_msg("P"..cp.." +"..got.." pts · "..node.name..rname, "good", 2.5)
    else
        set_msg("P"..cp.." moved to "..node.name, "info", 1.5)
    end

    if     route.obstacle=="NK"  then apply_nkabi(cp)
    elseif route.obstacle=="SNK" then apply_super_nkabi(cp)
    elseif route.obstacle=="POL" then apply_police(cp) end

    check_win()
    if state.game_over then return end

    state.set_count = state.set_count + 1
    local nxt = cp==1 and 2 or 1
    if state.turn_blocked[nxt] then
        state.turn_blocked[nxt] = false
        set_msg("P"..nxt.." was blocked! P"..cp.." moves again.", "warn", 2.5)
    else
...
function love.load()
    ...
    fonts.region = love.graphics.newFont(13)

    -- Load KZN map background if file exists (game works fine without it too)
    local ok, img = pcall(love.graphics.newImage, "kzn_map.png")
    if ok then mapImage = img end
...
local function draw_map_background()
    ...
    if mapImage then
        local iw = mapImage:getWidth()
        local ih = mapImage:getHeight()
        local pad = 20
        local s   = math.min((MAP_W-pad*2)/iw, (WIN_H-pad*2)/ih)
        local ox  = (MAP_W - iw*s) / 2
        local oy  = (WIN_H - ih*s) / 2
        -- Draw slightly desaturated and dim so nodes are readable
        love.graphics.setColor(1, 1, 1, 0.25)
        love.graphics.draw(mapImage, ox, oy, 0, s, s)
    end
...
-------------------------------------------------------------------------------
-- DRAW REGION LABELS (floating behind nodes)
-------------------------------------------------------------------------------
local function draw_region_labels()
    love.graphics.setFont(fonts.region)
    for _, reg in ipairs(REGIONS) do
        local sx, sy = tx(reg.lx, reg.ly)
        if sx>10 and sx<MAP_W-10 and sy>10 and sy<WIN_H-10 then
            local col  = reg.colour
            local name = reg.name:upper()
            local tw   = fonts.region:getWidth(name)
            -- Shadow
            love.graphics.setColor(0,0,0,0.60)
            love.graphics.print(name, sx-tw/2+1, sy+1)
            -- Label
            love.graphics.setColor(col[1],col[2],col[3],0.72)
            love.graphics.print(name, sx-tw/2, sy)
        end
    end
end
...
local function draw_routes()
    ...
    for _, r in ipairs(routes) do
        ...
        if r.cleared then sc(C.routeUsed) else sc(C.routeActive) end
...
-------------------------------------------------------------------------------
-- DRAW NODES  (region-coloured outer ring)
-------------------------------------------------------------------------------
local NR = 15
...
local function draw_nodes()
    ...
    for _, node in ipairs(nodes) do
        ...
        local reg  = NODE_REGION[node.id]
        local rcol = reg and reg.colour or {0.55, 0.55, 0.55}
        ...
        -- Flash glow on collect/hit
        if ft and ft>0 then
            sc(C.gold, ft/1.2*0.40)
            love.graphics.circle("fill", sx, sy, NR+10)
        end

        -- Region colour outer ring (always visible)
        love.graphics.setColor(rcol[1], rcol[2], rcol[3], 0.60)
        love.graphics.setLineWidth(3.5)
        love.graphics.circle("line", sx, sy, NR+3.5)
        ...
        -- Inner border (FIXED: proper line width handling)
        if is1 or is2 then
        ...
        elseif valid then 
            ...
            love.graphics.setColor(rcol[1], rcol[2], rcol[3], 0.85)
        end
...
local function draw_sidebar()
    ...
    sc(C.accent)
    love.graphics.printf("KZN ROUTE", px, y, pw, "center"); y=y+34
    love.graphics.setFont(fonts.small)
    sc(C.accentDim)
    love.graphics.printf("TAXI WARS", px, y, pw, "center"); y=y+24

    sc(C.accent, 0.22)
    love.graphics.rectangle("fill", SX+8, y, SIDEBAR_W-16, 1); y=y+12
    ...
    local function score_card(player, score, py)
        ...
        local preg  = pn and NODE_REGION[pn.id]
        local rcol  = preg and preg.colour or C.white
        local rname = preg and preg.name or ""

        rrect(px, py, pw, 78, 5, col, 0.09)
        sc(col, 0.30)
        love.graphics.setLineWidth(1.5)
        love.graphics.rectangle("line", px, py, pw, 78, 5, 5)
        love.graphics.setFont(fonts.small); sc(col)
        love.graphics.print("PLAYER "..player, px+8, py+7)
        love.graphics.setFont(fonts.large); sc(C.white)
        love.graphics.print(score.." pts", px+8, py+24)
        love.graphics.setFont(fonts.tiny); sc(C.white, 0.52)
        love.graphics.print("@ "..pname, px+8, py+48)
        -- Region pill
        love.graphics.setColor(rcol[1],rcol[2],rcol[3],0.30)
        love.graphics.rectangle("fill", px+8, py+60, pw-16, 11, 3,3)
        love.graphics.setColor(rcol[1],rcol[2],rcol[3],0.90)
        love.graphics.setFont(fonts.tiny)
        love.graphics.printf(rname, px+8, py+62, pw-16, "center")
        -- Progress bar
        local pct = math.max(0, math.min(score/20, 1))
        rrect(px+8, py+66, pw-16, 7, 3, C.panelLight)
        if pct>0 then rrect(px+8, py+66, (pw-16)*pct, 7, 3, col) end
    end

    score_card(1, state.p1_score, y); y=y+92
    score_card(2, state.p2_score, y); y=y+96

    love.graphics.setFont(fonts.tiny); sc(C.white, 0.33)
    love.graphics.printf("First to 20 points wins", px, y, pw, "center"); y=y+16

    sc(C.accent, 0.18)
    love.graphics.rectangle("fill", SX+8, y, SIDEBAR_W-16, 1); y=y+10

    -- Region legend
    love.graphics.setFont(fonts.tiny); sc(C.white, 0.50)
    love.graphics.printf("REGIONS", px, y, pw, "center"); y=y+14
    for _, reg in ipairs(REGIONS) do
        local col = reg.colour
        love.graphics.setColor(col[1],col[2],col[3],0.90)
        love.graphics.circle("fill", px+6, y+5, 5)
        sc(C.white, 0.68)
        love.graphics.setFont(fonts.tiny)
        love.graphics.print(reg.name, px+15, y)
        y = y + 13
    end
    y = y + 4

    sc(C.accent, 0.15)
    love.graphics.rectangle("fill", SX+8, y, SIDEBAR_W-16, 1); y=y+8
    ...
    if state.number_input ~= "" then
        ...
        love.graphics.setFont(fonts.tiny); sc(C.white, 0.36)
    ...
    -- Obstacles legend
    love.graphics.setFont(fonts.small); sc(C.white, 0.45)
    love.graphics.printf("OBSTACLES", px, y, pw, "center"); y=y+16
    ...
    local function leg(col, sym, label, desc, py)
        ...
        sc(C.white, 0.82); love.graphics.print(label, px+18, py+1)
        sc(C.white, 0.40); love.graphics.print(desc,  px+18, py+12)
    end

    leg(C.nkabi_c,  "N", "NKABI",       "lose 2 pts",    y); y=y+28
    leg(C.super_c,  "S", "SUPER NKABI", "back 3 nodes",  y); y=y+28
    leg(C.police_c, "P", "POLICE",       "skip next turn",y); y=y+28

    sc(C.routeUsed)
    love.graphics.setLineWidth(2.5)
    love.graphics.line(px, y+8, px+28, y+8)
    sc(C.white, 0.38); love.graphics.setFont(fonts.tiny)
    love.graphics.print("= used route", px+33, y+3); y=y+20

    -- Bottom controls hint
    y = WIN_H - 28
    sc(C.accent, 0.15)
    love.graphics.rectangle("fill", SX+8, y-8, SIDEBAR_W-16, 1)
    sc(C.white, 0.24); love.graphics.setFont(fonts.tiny)
...
local function draw_message()
    ...
    local col = C.accent
    if state.msg_type=="good" then col=C.p1
    elseif state.msg_type=="bad" then col=C.super_c
    elseif state.msg_type=="warn" then col=C.nkabi_c end
    local bw,bx,by = MAP_W-40, 20, WIN_H-50
...
function love.draw()
    ...
    -- Layer 1: map background (image + dot grid)
    draw_map_background()

    -- Layer 2: region name labels (behind routes/nodes)
    draw_region_labels()
